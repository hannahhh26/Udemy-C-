        ...
        public void Sort(Person[] people, Comparison<Person> comparison)
        {
            for (int i = 0; i < people.Length -1 ; i++)
            {
               for(int j = i+1; j < people.Length; j++)
                {
                    // Compare people[i] and people[j] using the comparison delegate
                    if (comparison(people[i], people[j]) > 0)
                    {
                        // swap people[i] and people[j] if they are in wrong order of sorting algorithms e.g. alphabet
